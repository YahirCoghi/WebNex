...
export function Services() {
  ...
  const items = (t.raw("items") as ServiceItem[]).slice(0, 3);
          ...
          {items.map((service, index) => {
            ...
            return (
              ...
              >
                ...
                {!featured ? <span className="mb-4 inline-flex text-[11px] font-semibold uppercase tracking-[0.16em] text-[#4a72e6]">{service.id}</span> : null}
                {featured ? <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#4a72e6]">{service.id}</p> : null}
